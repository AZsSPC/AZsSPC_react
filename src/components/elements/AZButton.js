...
function AZButton({ children, color, classes = '', pure = false, ...props }) {
    return <button className={`az-button ${classes} ${color ? `color-${color}` : ''}  ${pure ? 'pure' : ''}`} {...props}>{children}</button>;
}

function AZButtonCopy({ children, copy, classes = '', color, pure = false, ...props }) {
    const notify = useNotify();
    return <button className={`az-button az-button-copy ${classes} ${color ? `color-${color}` : ''}  ${pure ? 'pure' : ''}`} {...props}
        onClick={(e) => {
